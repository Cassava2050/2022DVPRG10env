...
# ixing yield data of 202082DVPRG_pivi
...
plot_bxp <- trial_tidy %>%
  ...
  theme_xiaofei() +
  theme(
    axis.text.x = element_text(size = 7, vjust = 1, angle = 65),
...
if (group == "group1"){
  my_dat_1 <- my_dat[!my_dat$trial_name %in% c("202136DVPRG_ciat", "202050DVPRG_ciat"), ]
}
...
if(group == "group1") { 
elements_to_remove <- c("202050DVPRG_ciat", "202136DVPRG_ciat") # from group 1
trials <- unique(my_dat$trial_name)[!unique(my_dat$trial_name) %in% elements_to_remove]
} else {
...
if(group == "group1"){ 
PALMIRA <- my_dat %>% filter(trial_name %in% c("202050DVPRG_ciat", "202136DVPRG_ciat")) %>% 
  select(accession_name, trial_name, root_type1_5) %>% 
  filter(!accession_name %in% c("GM10055-1", "GM10055-2", "SM1127-8_is_Cubana")) %>% 
  pivot_wider(names_from = trial_name, values_from = root_type1_5)


master_data$BLUP_BLUE_root_type1_5 <- master_data$BLUP_BLUE_root_type1_5 %>% 
  left_join(PALMIRA, by = c("genotype" = "accession_name")) %>% 
...
colnames(PALMIRA)[-1] <- paste("BLUEs_root_type1_5", colnames(PALMIRA)[-1], sep="_")
  
BLUEs_BLUPs <- 
  BLUEs_BLUPs %>% left_join(PALMIRA, by = c("genotype" = "accession_name"))
}
...
meta_file_name <- paste0(folder_output, paste("2022", trial_interest, group, "master_results", Sys.Date(), ".xlsx", sep = "_"))

grep("^stability_root_number_commercial", names(master_data))

names(master_data)[13] <- "BLUP_BLUE_germ_number_plot"
names(master_data)[22] <- "BLUP_BLUE_root_commercial"
names(master_data)[35] <- "stability_germ_number_plot"
names(master_data)[44] <- "stability_root_commercial"
