...
```{r}
if(FALSE){ trial_standard <- trial_standard %>% 
...
From here I'm going to create an extra branch to carry out one group of the analysis 

# First group 

First group is compound by 202118DVPRG_momi, 202121DVPRG_repe, 202081DVPRG_momi, 202082DVPRG_pivi


```{r}
trial_tidy_all <- trial_tidy_all %>% filter(trial_name %in% c("202118DVPRG_momi", "202121DVPRG_repe", "202081DVPRG_momi", "202082DVPRG_pivi"))
...
traits_for_analysis <- c("branch_number", "DM_gravity", "height_1st_branch", "yield_ha", "vigor1_5", 
  "plant_type", "height", "root_skin_color1_3", "root_type1_5", # "carotenoid1_8", 
  "germination_perc", "DM_yield_ha") 

trait_ideal <- analysis_trait[analysis_trait %in% traits_for_analysis]
...
# ggsave(paste0("images\\pheno_corr_", trial_interest, "_", group, "_", Sys.Date(), ".png"),
...
ggsave(paste0('images\\shared_', trial_interest, "_", group, "_", Sys.Date(), ".png"),
...
p1
# ggsave(paste0("images\\missing_", trial_interest, "_", group, "_", Sys.Date(), ".png"),
...
stab <- met_results$stability %>% 
  ...
  filter(genotype %in% shared_genos_2021)
...
stab_values <- master_data[c(21:31)] %>% 
...
meta_file_name <- paste0(folder_output, paste0("2022", trial_interest, "4env", "_master_results_", Sys.Date(), "_.xlsx" ))
...
```{r}
blupDF_kp <- blupDF_kp %>% 
  filter(accession_name %in% shared_genos_2021)
```
...
TestIndex_ <- index_dat %>%
  ...
  drop_na()
res.pca <- pca_index(data = TestIndex_, id = "accession_name", b = c(10, -5, 10), percentage = 0.20)
...
```{r}
write.csv(selections, here::here("output", paste("01", year_interest, trial_interest, "4env",
  "selection", Sys.Date(), ".csv", sep = "_")), row.names = FALSE)
```
